        ...
        try:
            ...
            if image is not None:
                ...
                self.get_logger().debug(
                    f"Decoded image with original shape: {original_shape}"
                )

                # Resize image if enabled and different from target resolution
                if self.resize_enabled:
                    if (
                        image.shape[1] != self.output_width
                        or image.shape[0] != self.output_height
                    ):
                        self.get_logger().debug(
                            f"Resizing from {image.shape[1]}x{image.shape[0]} to {self.output_width}x{self.output_height}"
                        )
                ...
                try:
                    ...
                    self.get_logger().debug(
                        f"Published image: {image.shape[1]}x{image.shape[0]} to /camera/image_raw"
                    )
                ...
                self.get_logger().error(
                    "Failed to decode image data - cv2.imdecode returned None"
                )
