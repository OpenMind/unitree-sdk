    ...
    def timer_callback(self):
        ...
        if self.latest_tag is None or time_since_seen > self.lost_threshold:
            ...
            if self.search_turn_side == 0:  
                ...
                if self.search_turn_deg > self.max_search_deg:
                    ...
                    if self.continuous_back_count == 3:  # third time, move bigger step to escape 
        ...
        if int(see_angle) == 1:  # see target 
            if abs(bearing_angle) > 10.0:
                x_cmd = 0.5*x_cmd 
                yaw_cmd_degree = max(-9, min(9, 0.8*bearing_angle))   # proportional adjustment change 30 degree to radian 
                yaw_cmd_radian = math.radians(yaw_cmd_degree)*(-1)    # becaues unitree turn left is positive    
        ...
        if in_forward and in_right:
            self.zone_counter += 1
            self.get_logger().debug(f"In charging zone candidate ({self.zone_counter}/3)")
