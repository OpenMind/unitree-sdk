...
class APIHandlers:
    ...
    def register_routes(self, app):
        ...
        def start_base_control():
            ...
            """
            if self.orchestrator.is_slam_running() or self.orchestrator.is_nav2_running():
        ...
        def start_slam():
            ...
            """
            if self.orchestrator.nav2_manager.process and self.orchestrator.nav2_manager.process.poll() is None:
        ...
        def start_nav2():
            ...
            """
            if self.orchestrator.slam_manager.process and self.orchestrator.slam_manager.process.poll() is None:
        ...
        def status():
            ...
            return jsonify({"status": "success", "message": json.dumps(status_data.model_dump())}), 200
        ...
        def delete_map():
            ...
            if result["status"] == "success":
                ...
                return jsonify(result), 404 if "does not exist" in result["message"] else 500
        ...
        def add_map_location():
            ...
            map_name = data['map_name']
            result = self.orchestrator.location_manager.add_location(map_name, validated_location)
        ...
        def list_map_locations():
            ...
            return jsonify({"status": "success", "message": json.dumps({name: loc.model_dump() for name, loc in locations.items()})}), 200
        ...
        def add_slam_location():
            ...
            if result["status"] == "success":
                if self.orchestrator.get_logger():
                    self.orchestrator.get_logger().info(f"Saved location '{label}' at position ({pose['position']['x']:.2f}, {pose['position']['y']:.2f})")
