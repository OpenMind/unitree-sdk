...
class CmdVelToG1LocoNode(Node):
    ...
    def __init__(self):
        ...
        self.max_angular_velocity = 0.5   # rad/s rotation
        ...
        self.get_logger().info(f"Max velocities - X: {self.max_linear_x_velocity} m/s, "
    ...
    def cmd_vel_callback(self, msg: Twist):
        ...
        linear_x = max(-self.max_linear_x_velocity, 
                      min(self.max_linear_x_velocity, linear_x))
        linear_y = max(-self.max_linear_y_velocity, 
                      min(self.max_linear_y_velocity, linear_y))
        angular_z = max(-self.max_angular_velocity, 
                       min(self.max_angular_velocity, angular_z))
        
        # Check if this is effectively a stop command
        velocity_threshold = 0.001
        if (abs(linear_x) < velocity_threshold and 
            abs(linear_y) < velocity_threshold and 
            abs(angular_z) < velocity_threshold):
    ...
    def check_timeout_callback(self):
        ...
        if self.is_moving and time_since_last_cmd > self.movement_timeout:
            self.send_stop_command()
            self.is_moving = False
            self.get_logger().warn(f"Movement timeout ({self.movement_timeout}s) - stopping robot")
