...
class AprilTagNode(Node):
    ...
    def __init__(self):
        ...
        # Load intrinsics directly from YAML
        yaml_path = "/home/openmind/camera_ws/src/my_camera_pkg/config/unitree_go2_cam_indiana/camera_info.yaml"
    ...
    def load_camera_intrinsics(self, yaml_file):
        ...
        self.camera_params = [fx, fy, cx, cy]
        self.get_logger().info(f"Loaded intrinsics from {yaml_file}: fx={fx}, fy={fy}, cx={cx}, cy={cy}")
            ...
            if tag_id1 in tag_dict and tag_id2 in tag_dict:
                ...
                if v12_norm > 0:
                    ...
                    if wall_forward_norm > 0:
                        ...
                        distance_confidence = max(0.1, 1.0 - distance_error)  # Higher when measurement matches known distance
                        
                        # 3. Geometric stability (longer baselines generally better for bearing)
                        baseline_confidence = min(2.0, known_distance / 0.05)  # Normalize to reasonable range
